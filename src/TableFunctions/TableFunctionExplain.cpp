...
void TableFunctionExplain::parseArguments(const ASTPtr & ast_function, ContextPtr /*context*/)
{
    ...
    {
        if (const auto * explain_query = function->arguments->children[0]->as<ASTExplainQuery>())
            query = explain_query->clone();
    }

    if (!query)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Table function '{}' requires a explain query argument", getName());
}
